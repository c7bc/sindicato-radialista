...
export function transformSiteToHeroContent(site: PayloadSite): HeroContent {
  return {
    ...
    badgeText: site.hero?.badgeText || 'Junte-se à força dos Radialistas',
    title: site.hero?.title || 'Fortaleça sua voz. Fortaleça sua categoria.',
    description: site.hero?.description || 'O Sindicato dos Radialistas é mais do que uma entidade: é o eco da sua dignidade profissional.',
    ...
    imageAlt: site.hero?.imageAlt || 'Radialista em estúdio de rádio',
  }
}
...
export function transformSiteToFooterContent(site: PayloadSite): FooterContent {
  ...
  return {
    ...
    description: site.footer?.description || 'Representando e defendendo os direitos dos radialistas com força e dedicação.',
